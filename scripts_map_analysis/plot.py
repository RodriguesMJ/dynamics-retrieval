...
def multitape_bR(neg_integral, pos_integral, nmaps, path, label):
    ...
    if '0_4' in label: 
        m = 4         
        start_end_list = [[760, 860]]#[[690, 800]]#[[435, 690], [500, 690], [690, 1050]]
    if '0_3' in label: 
        m = 3
    ...
    for start_end in start_end_list:
        ...
        matplotlib.pyplot.vlines(start, numpy.min(data_all), 0, linestyles="--", color="0.2")
        matplotlib.pyplot.vlines(end, numpy.min(data_all), 0, linestyles="--", color="0.2")
        matplotlib.pyplot.hlines(numpy.min(data_all), start, end, linestyles="--", color="0.2")
        ...
        print 'Max at:', tick_function_THz_to_wavens(freq[numpy.argmax(spec)]/(10**12)), 'cm-1'
...
if __name__ == '__main__':
    ...
    for mode in modes:
        for sigcutoff in sigcutoffs:
            ...
            if flag_bR == 1:
                print 'Mode', mode
                path = '/das/work/p17/p17491/Cecilia_Casadei/NLSA/data_bR/map_analysis/maptool_CMC/output_figures/out_mode_0_%d_h5grid_8'%mode
                label = 'map_modes_0_%d_radius_%.1f_dist_%.1f_sig_%s'%(mode, radius, distance, sigcutoff)
