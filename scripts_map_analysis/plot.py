...
def multitape(neg_integral, pos_integral, nmaps, path, label):
    ...
    for start_end in start_end_list:
        ...
        path_figs = "%s/multitape_m_0_%d_fcrit_%.2f_range_%d_%d" % (
            path,
            m,
            fcrit_value,
            start,
        ...
        ax1.text(
            ...
            0.95,
            "Modes: %s + %s, %s sigma, %s"
            % (label[2], label[4], label[36:39], label[40:44]),
        ...
        new_tick_locations = numpy.array(
            [start - start, (start - start + end - start) / 2, end - start]
        )
        ...
        ax2.fill_between(
            freq / (10 ** 12), jackknife[:, 0], jackknife[:, 1], color="blue", alpha=0.3
        )
        ax2.set_xlim(0, xmax)
        ...
        for p in [100 * fcrit_value]:
            ...
            matplotlib.pyplot.text(
                x=1, y=y + 0.2, s="%i %%" % p, ha="left", fontsize=25
            )
        ...
        matplotlib.pyplot.savefig(
            "%s/multitape_%s_zoom_%d_%d_F_stats.png" % (path_figs, label, start, end)
        )
        matplotlib.pyplot.close()
...
def multitape_zoom(neg_integral, pos_integral, nmaps, path, label):
    ...
    new_tick_locations = numpy.array(
        [
            (t_idx_left - t_idx_left) / step,
            (t_idx_left - t_idx_left + t_idx_right - t_idx_left) / (2 * step),
            (t_idx_right - t_idx_left) / step,
        ]
    )
    ax1_t.set_xticks(new_tick_locations)
    ax1_t.set_xticklabels(tick_function(new_tick_locations))
    ...
    matplotlib.pyplot.tight_layout()
    matplotlib.pyplot.savefig(
        "%s/multitape_%s_zoom_%d_%d_F_stats.png"
        % (path_figs, label, t_idx_left, t_idx_right)
    )
    matplotlib.pyplot.close()
...
def plot_atoms_density(meanposden, meannegden, path, label, atoms):
    ...
    nmaps = meannegden.shape[1]
    print "integrated meanposden: ", whole_posden.shape
    print "integrated meannegden: ", whole_negden.shape
    plot(whole_negden, whole_posden, nmaps, path, label)
    ...
    multitape_bR(whole_negden, whole_posden, nmaps, path, label)


def multitape_bR(neg_integral, pos_integral, nmaps, path, label):
    # def tick_function(X):
    #     step_fs = 100*(385.0+335.0)/213557
    #     V = (X+start-t0)*step_fs
    #     return ["%.3f" % z for z in V]
    ...
    if "0_3" in label:
        m = 3
        start_end_list = [[100, 400]]  # [[500, 1000]]#, [1780, 1950]]
    ...
    for start_end in start_end_list:
        ...
        data_all = neg_integral
        c = "m"
        # data_all = pos_integral
        # c = 'b'

        data = data_all[start:end]

        # bR
        time_window_left = -56.4
        time_window_right = 1133.0
        time_window_n = 99169

        # # Rho
        # time_window_left = -335.0
        # time_window_right = 385.0
        # time_window_n = 213557

        step = 100
        # delta_s = (10**(-15))*step*(time_window_right-time_window_left)/time_window_n #s
        delta_s = (10 ** (-15)) * step * 978.4 / 83784
        xmax = 200  # THz, for figures

        fig = matplotlib.pyplot.figure(figsize=(25, 40))

        # TIME DOMAIUN
        ax1 = fig.add_subplot(6, 1, 1)
        # ax1.set_xlim(start, end)
        # ax1.plot(range(start, end), data, color='m')
        ax1.set_xlim(0, data_all.shape[0])
        ax1.plot(range(data_all.shape[0]), data_all, color=c)
        matplotlib.pyplot.vlines(
            start, numpy.min(data_all), 0, linestyles="--", color="0.2"
        )
        matplotlib.pyplot.vlines(
            end, numpy.min(data_all), 0, linestyles="--", color="0.2"
        )
        matplotlib.pyplot.hlines(
            numpy.min(data_all), start, end, linestyles="--", color="0.2"
        )
        matplotlib.pyplot.hlines(0, start, end, linestyles="--", color="0.2")
        ...
        print "Max at:", freq[numpy.argmax(spec)] / (10 ** 12), "THz"
        print "Max at:", tick_function_THz_to_wavens(
            freq[numpy.argmax(spec)] / (10 ** 12)
        ), "cm-1"
        print freq[10] / (10 ** 12) - freq[9] / (10 ** 12), "THz error"


if __name__ == "__main__":
    radius = 1.7
    distance = 0.2
    sigcutoffs = [4]  # [2.5, 3.0, 3.5, 4.0, 4.5]
    modes = [4]
    ...
    for mode in modes:
        for sigcutoff in sigcutoffs:
            ...
            if flag_bR == 1:
                ...
                label = "map_modes_0_%d_radius_%.1f_dist_%.1f_sig_%s" % (
                    mode,
                    radius,
                    distance,
                    sigcutoff,
                )
                fn_n = "%s/meannegden_%s.mat" % (path, label)
                mat = scipy.io.loadmat(fn_n)
                meannegden = mat["meannegden"].T
                fn_p = "%s/meanposden_%s.mat" % (path, label)
                mat = scipy.io.loadmat(fn_p)
                meanposden = mat["meanposden"].T
                print "meanposden: ", meanposden.shape
                print "meannegden: ", meannegden.shape
            ...
            if flag_bR == 1:
                ...
                # HELIX G
                # atoms = range(1516, 1717)
                # label = '%s_helix_G'%label

                plot_atoms_density(meanposden, meannegden, path, label, atoms)
