...
def f(x, A, B):
    return A + B*numpy.cos(2*numpy.pi*x*t_d_x)
...
def fit(Is_avg, h_range):
    
    xdata = range(h_range)
    ydata = numpy.asarray(Is_avg)[0:h_range]
...
def correct(f_w, f_r, A, B):
    ...
    for i in fo_r:        
        ...
        factor = 1.0/( A + B * numpy.cos( 2 * numpy.pi * h * t_d_x ) )
        I_corr = factor*I
        #sigI_corr = factor*sigI
        fo_w.write('%4d%5d%5d%15.2f\n'%(h, k, l, I_corr))
...
def plot(I_avg, fign):
    ...
    matplotlib.pyplot.xlabel(r'h', fontsize=22),
    matplotlib.pyplot.ylabel(r'<I>_h', fontsize=22)
...
if __name__ == '__main__':  
    ...
    # Rewrite original in different format F -> I
    filename = '%s/%s.txt'%(path, label)
    fo_w_fn_original = '%s/%s_original_Is.txt'%(path, label)
    rewrite(fo_w_fn_original, filename)
       
    # Get average I vs h    
    Is_avg = get_I_avg_vs_h(fo_w_fn_original)
    ...
    fn_r = '%s/%s_original_Is.txt'%(path, label)
...
### METHOD 1: FIT ###
    ...
    # Fit 
    h_max = 10 # For fitting purposes
    A, B, fraction_fit = fit(Is_avg, h_max)
    ...
    # Correct intensities
    fn_w = '%s/%s_corrected_Is.txt'%(out_path, label)
    correct(fn_w, fn_r, A, B)
    
    # Get average corrected I vs k
    Is_avg = get_I_avg_vs_h(fn_w)
