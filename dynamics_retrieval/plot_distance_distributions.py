...
def plot_distributions(settings):
    ...
    matplotlib.pyplot.savefig(
        "%s/D_lp_filtered_fmax_%d_unique_hist.png" % (results_path, f_max)
    )
    ...
    matplotlib.pyplot.savefig(
        "%s/D_lp_filtered_fmax_%d_unique_and_nns_hist_log.png" % (results_path, f_max)
    )
    matplotlib.pyplot.close()
...
def plot_D_0j_lp_filter(settings):
    ...
    for idx in idxs:
        ...
        ax.tick_params(axis="both", which="minor", labelsize=20)
        matplotlib.pyplot.savefig(
            "%s/D_%d_j_lp_filtered_fmax_%d.png" % (results_path, idx, f_max)
        )
        matplotlib.pyplot.close()
...
def plot_D_0j_lp_filter_zoom(settings):
    ...
    for idx in idxs:
        ...
        ax.set_ylim([125, 175])
        matplotlib.pyplot.savefig(
            "%s/D_%d_j_lp_filtered_fmax_%d_zoom.png" % (results_path, idx, f_max)
        )
        matplotlib.pyplot.close()
...
def plot_D_0j_recap_log(settings):
    ...
    for i, f_max in enumerate(f_max_lst):
        ...
        D_0j = numpy.sqrt(
            joblib.load("%s/D_sq_lp_filtered_fmax_%d.jbl" % (results_path, f_max))[0, :]
        )
        print "D_0j: ", D_0j.shape, D_0j.dtype
...
def plot_D_0j_recap_lin(settings):
    ...
    for i, f_max in enumerate(f_max_lst):
        idx = 100000  # 0
        D_0j = numpy.sqrt(
            joblib.load("%s/D_sq_lp_filtered_fmax_%d.jbl" % (results_path, f_max))[
                idx, :
            ]
        )
        print "D_0j: ", D_0j.shape, D_0j.dtype
...
def plot_D_matrix(settings):
    ...
    matplotlib.pyplot.imshow(
        numpy.log10(D + 1), cmap="jet", vmin=0, vmax=numpy.log10(p99)
    )
...
def plot_D_submatrix(settings):
    ...
    D = numpy.asarray(D, dtype=numpy.float32)
    matplotlib.pyplot.imshow(
        numpy.log10(D + 1), cmap="jet", extent=[col_min, col_max, row_max, row_min]
    )
    matplotlib.pyplot.colorbar()
    matplotlib.pyplot.savefig(
        "%s/D_submatrix_%d_%d_%d_%d_lp_filtered_fmax_%d.png"
        % (results_path, row_min, row_max, col_min, col_max, f_max)
    )
    matplotlib.pyplot.close()
