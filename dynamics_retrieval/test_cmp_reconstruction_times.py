...
def get_t_r_large(q):
    ...
    if p == 0:
        ...
        t_r_large[(q-1)/2:(q-1)/2+(S-q+1),] = t_r
        return t_r_large 
    else:
...
def main(settings, qs):
    ...
    for q in qs[1:]:
        ...
        start = start_large[(q_max-1)/2:(q_max-1)/2+(S-q_max+1),]
        t_r   =   t_r_large[(q_max-1)/2:(q_max-1)/2+(S-q_max+1),]
    ...
    joblib.dump(avgs, '%s/LPSA_f_max_%d_q_scan_p_%d_successive_abs_dt_avg.jbl'
    ...
    for i, avg in enumerate(avgs):
        matplotlib.pyplot.scatter(i+1, 
                                  avgs[i], 
                                  c=colors[i], 
                                  label='q=%d,q=%d'%(qs[i], qs[i+1]))
    matplotlib.pyplot.legend(frameon=True, 
                             scatterpoints=1, 
                             loc='upper right', 
                             fontsize=6) 
    matplotlib.pyplot.savefig('%s/LPSA_f_max_%d_q_scan_p_%d_successive_abs_dt_avg.png'
                               %(results_path, f_max, p), dpi=96*4)
