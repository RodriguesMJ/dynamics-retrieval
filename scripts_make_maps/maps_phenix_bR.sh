...
phenix.fobs_minus_fobs_map f_obs_1_file=light-F.mtz f_obs_2_file=dark-F.mtz f_obs_1_label=F,SIGF f_obs_2_label=F,SIGF phase_source=${PHASE_model} high_res=${RESO} low_res=20.0 multiscale=True output_file=FoFoPHFc_Phenix.mtz

fft hklin FoFoPHFc_Phenix.mtz mapout light--dark.ccp4<<EOF
LABIN F1=FoFo PHI=PHFc
#RESOLUTION 4.5 ${RESO}
RESOLUTION 20.0 ${RESO}
GRID SAMPLE 8
END
EOF
