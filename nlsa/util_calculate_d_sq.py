...
def f_dense(x, mask):
    print '\n****** RUNNING calculate_d_sq DENSE INPUT ******'
    m = x.shape[0]    
    S = x.shape[1]
...
def calc_x_sq_T_mask_term(x_sq, mask):
    import settings_rho_light as settings
    mask = mask[:,:].todense()
    mask = numpy.asarray(mask, dtype=numpy.uint8)
    x_sq = x_sq[:,:].todense()
    x_sq = numpy.asarray(x_sq, dtype=numpy.float32)
    ...
    print 'x_sq_T_mask:', x_sq_T_mask.dtype
...
def calc_mask_T_x_sq_term(x_sq, mask):
    import settings_rho_light as settings
    mask = mask[:,:].todense()
    mask = numpy.asarray(mask, dtype=numpy.uint8)
    x_sq = x_sq[:,:].todense()
    ...
    print 'mask_T_x_sq:', mask_T_x_sq.dtype
...
def f_sparse_x_sq_T_mask(x, mask):
    ...
    calc_x_sq_T_mask_term(x_sq, mask)
    
def f_sparse_mask_T_x_sq(x, mask):
    ...
    calc_mask_T_x_sq_term(x_sq, mask)
...
def f_add_2():   
    ...
    temp += to_add
    print 'Saving d_sq'    
    joblib.dump(temp, '%s/d_sq.jbl'%settings.results_path)
