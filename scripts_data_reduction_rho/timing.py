...
def get_ts_distribution():   
    ...
    for scan_n in range(1, 24):
        ...
        for run_n in range(run_start, run_end+1):
            ...
            list_file_light = '%s/rho_nlsa_scan_%d/run_%0.6d.JF06T08V01.light.lst'%(path, scan_n, run_n)
            ...
            if not (pid_BSREAD.shape == pid_SPECENC.shape and 
                ...
                if (diff_1.sum() > 0 or diff_2.sum() > 0):
                    ...
                    n_selected_runs += 1
                    print 'RUN: ',  run_n, ', ', pid_BSREAD.shape[0], ' frames.' # 1000 frames (from 0 to 999) except last run of scan.
    ...
    plt.title('Arrival times amplitude (light shot timestamp reliability.) N.: %d'%ts_rel_light.shape[0])
...
def export_unique_ID():
    ...
    for scan_n in range(1, 24):
        ...
        for run_n in range(run_start, run_end+1):
            try:
                data = joblib.load('%s/light_scan_%d_run_%0.6d_event_pid_ts_tsrel_felI.jbl'
                                    %(path_out, scan_n, run_n))
                n_frames = data.shape[0]
                path_string = '/sf/alvra/data/p18594/raw/rho_nlsa_scan_%d/run_%0.6d.JF06T08V01.h5_event_'%(scan_n, run_n)
                unique_IDs = []                                                                                           
                for i in range(n_frames):
                     event_n = str(int(data[i, 0]))
                     unique_ID = path_string + event_n
                     unique_IDs.append(unique_ID)
                joblib.dump(unique_IDs, '%s/light_scan_%d_run_%0.6d_uniqueID.jbl'
                                         %(path_out, scan_n, run_n))  
    ...
    for scan_n in range(1, 24):
        ...
        for run_n in range(run_start, run_end+1):
            try:
                ...
                data = joblib.load('%s/light_scan_%d_run_%0.6d_event_pid_ts_tsrel_felI.jbl'
                                    %(path_out, scan_n, run_n))
                unique_IDs = joblib.load('%s/light_scan_%d_run_%0.6d_uniqueID.jbl'
                                          %(path_out, scan_n, run_n))
        ...
        scan_ts_rel_flat = numpy.asarray([item for sublist in scan_ts_rel for item in sublist])
        scan_FEL_Is_flat = numpy.asarray([item for sublist in scan_FEL_Is for item in sublist])
        ...
        joblib.dump(scan_out, 
                    '%s/light_scan_%d_ts_tsrel_felI.jbl'
                     %(path_out, scan_n))
        
        joblib.dump(scan_uniqueIDs_flat, 
                    '%s/light_scan_%d_uniqueID.jbl'
                     %(path_out, scan_n))  
...
def twoD_Gaussian_rotated_bimodal((x, y), 
                                  amplitude_1, x0_1, y0_1, sigma_x_1, sigma_y_1, theta_1,
                                  amplitude_2, x0_2, y0_2, sigma_x_2, sigma_y_2, theta_2):  
    
    a_1 = (numpy.cos(theta_1)**2)/(2*sigma_x_1**2) + (numpy.sin(theta_1)**2)/(2*sigma_y_1**2)
    b_1 = (-numpy.sin(2*theta_1))/(4*sigma_x_1**2) + (numpy.sin(2*theta_1))/(4*sigma_y_1**2)
    c_1 = (numpy.sin(theta_1)**2)/(2*sigma_x_1**2) + (numpy.cos(theta_1)**2)/(2*sigma_y_1**2)
    dx_1 = x-x0_1
    dy_1 = y-y0_1
    expo_1 = a_1*dx_1**2 + 2*b_1*dx_1*dy_1 + c_1*dy_1**2
    
    a_2 = (numpy.cos(theta_2)**2)/(2*sigma_x_2**2) + (numpy.sin(theta_2)**2)/(2*sigma_y_2**2)
    b_2 = (-numpy.sin(2*theta_2))/(4*sigma_x_2**2) + (numpy.sin(2*theta_2))/(4*sigma_y_2**2)
    c_2 = (numpy.sin(theta_2)**2)/(2*sigma_x_2**2) + (numpy.cos(theta_2)**2)/(2*sigma_y_2**2)
    dx_2 = x-x0_2
    dy_2 = y-y0_2
...
def select():
    ...
    for scan_n in range(1, 24):
        ...
        scan_data = joblib.load('%s/light_scan_%d_ts_tsrel_felI.jbl'
                                 %(path_out, scan_n))
        
        scan_unique_IDs = joblib.load('%s/light_scan_%d_uniqueID.jbl'
                                       %(path_out, scan_n)) 
        ...
        #plt.xlabel('FEL Intensity [a.u.] (SAROP11-PBPS122:INTENSITY)')
        #plt.ylabel('Arrival times amplitude (light shot timestamp reliability.)')
        plt.savefig('%s/scan_%d_plot_light_tsrel_vs_FEL_Is_lims.png'%(path_out, scan_n))
        plt.close() 
        ...
        for i in range(n_pts_y):
            ...
            for j in range(n_pts_x):
                ...
                idxs = numpy.argwhere((dI**2)/(a**2) + (dtsrel**2/(b**2)) < 1)
                n_nn = idxs.shape[0]
                #print x, y, n_nn
                if n_nn == 0:
                    n_nn = 1 # to take ln
        ...
        #FIGURES
        plt.scatter(x_grid.flatten(),  y_grid.flatten(),  c=density_grid.flatten(), s=5, alpha=0.3)    
        plt.title('Scan %d'%scan_n)
        plt.xlabel('FEL Intensity [a.u.] (SAROP11-PBPS122:INTENSITY)')
        plt.ylabel('Arrival times amplitude (light shot timestamp reliability.)')
        plt.savefig('%s/scan_%d_density_plot_light_tsrel_vs_FEL_Is.png'%(path_out, scan_n))
        plt.close()
        
        plt.scatter(x_grid.flatten(),  y_grid.flatten(),  c=ln_density_grid.flatten(), s=5, alpha=0.3)    
        plt.title('Scan %d'%scan_n)
        plt.xlabel('FEL Intensity [a.u.] (SAROP11-PBPS122:INTENSITY)')
        plt.ylabel('Arrival times amplitude (light shot timestamp reliability.)')
        plt.savefig('%s/scan_%d_ln_density_plot_light_tsrel_vs_FEL_Is.png'%(path_out, scan_n))
        plt.close()
         
        # Fit ln(density) with uni/bi-modal 2D elliptical Gaussian.
        initial_i_idx = numpy.unravel_index(numpy.argmax(ln_density_grid, axis=None), ln_density_grid.shape)[0] 
        initial_j_idx = numpy.unravel_index(numpy.argmax(ln_density_grid, axis=None), ln_density_grid.shape)[1] 
        ...
        if scan_n == 8:
            initial_guess = (initial_amplitude,   2.6, 0.11, 0.15, 0.01/3, initial_theta/10,
                             initial_amplitude/2, 3.1, 0.10, 0.25, 0.01/2, initial_theta/10)            
            f = twoD_Gaussian_rotated_bimodal
            thresh = 0.1            
        elif scan_n == 14:
            initial_guess = (initial_amplitude,   initial_x, 0.13, 0.5, 0.01/3, initial_theta/100,
                             initial_amplitude/2, initial_x, 0.08, 0.5, 0.01/3, initial_theta/100)            
            f = twoD_Gaussian_rotated_bimodal
            thresh = 0.1            
        elif scan_n == 20:
            initial_guess = (initial_amplitude,   2.6, 0.125, 0.2, 0.01/2, initial_theta/100,
                             initial_amplitude/2, 3.1, 0.125, 0.2, 0.01/2, initial_theta/100)           
            f = twoD_Gaussian_rotated_bimodal
            thresh = 0.1                    
        else:
            initial_guess = (initial_amplitude, initial_x, initial_y, 0.5, 0.01, initial_theta)
            f = twoD_Gaussian_rotated
            thresh = 0.05
            
        popt, pcov = scipy.optimize.curve_fit(f, 
                                              (x_grid, y_grid), 
                                              ln_density_grid.flatten(), 
                                              p0=initial_guess)
        ...
        plt.xlim((1.0,4.0))
        plt.ylim((0.0,0.2))
        plt.gca().contour(x_grid, y_grid, data_fitted.reshape(n_pts_y, n_pts_x), 4, colors='w')
        plt.title('Scan %d'%scan_n)
        #plt.xlabel('FEL Intensity [a.u.] (SAROP11-PBPS122:INTENSITY)')
        #plt.ylabel('Arrival times amplitude (light shot timestamp reliability.)')
        plt.savefig('%s/scan_%d_ln_density_plot_light_tsrel_vs_FEL_Is_fit_rotated.png'%(path_out, scan_n))
        plt.close()
        ...
        joblib.dump(scan_data_selected, '%s/light_scan_%d_ts_tsrel_felI_selected.jbl'
                                          %(path_out, scan_n))    
        joblib.dump(scan_unique_IDs_selected, '%s/light_scan_%d_uniqueID_selected.jbl'
                                                %(path_out, scan_n))         
        print 'NaNs in selected light data: ', numpy.any(numpy.isnan(scan_data_selected))
        ...
        #plt.xlabel('FEL Intensity [a.u.] (SAROP11-PBPS122:INTENSITY)')
        #plt.ylabel('Arrival times amplitude (light shot timestamp reliability.)')
        plt.savefig('%s/scan_%d_plot_light_tsrel_vs_FEL_Is_selected.png'%(path_out, scan_n))
        plt.close()
...
def check_t_distribution():
    ...
    for scan_n in range(1, 24):
        ...
        print '\n'
        print 'SCAN: ', scan_n
        scan_data_selected = joblib.load('%s/light_scan_%d_ts_tsrel_felI_selected.jbl'
                                          %(path_out, scan_n))   
        print '\n'    
...
def export_unique_ID_ts():
    ...
    for scan_n in range(scan_start, scan_end+1):
        ...
        data_ts = joblib.load('%s/light_scan_%d_ts_tsrel_felI_selected.jbl'%(path_out, scan_n))
        ts = data_ts[:,0]    
        n_frames = ts.shape[0]
                
        data_IDs = joblib.load('%s/light_scan_%d_uniqueID_selected.jbl'%(path_out, scan_n))
