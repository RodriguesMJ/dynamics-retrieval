...
folder = '';
fn = [path, folder, 'partialator_OK_', label, '.mat']; 
...
%vertices (h,k,l).
    ...
    temp_vol(hh+1,kk+1,ll+1) = intensity + temp_vol(hh+1,kk+1,ll+1); 
...
%we add intensities on top of each other (needed for intensity normalization of vertices) 
    ...
    count(hh+1,kk+1,ll+1) = 1 + count(hh+1,kk+1,ll+1);
...
% to save
filename = sprintf('data_rho_%s_nS%d_nBrg%d.mat', ...
                   label,...
                   num_snapshot,...
